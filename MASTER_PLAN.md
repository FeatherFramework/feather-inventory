...
**Since 2026-08-23 the whole `INV-W1`–`W4` weapons-support track has landed, and then been reworked once by a review from `feather-weapons` itself** (§6.1) — item instances with `stack`/`unique` modes, versioned metadata documents, real row-locked transactions, a movement-guard registry, structured post-commit events, persisted equipment slots, and decimal item weights. One of those changes is a correction to this document rather than to the code: the claim that `oxmysql` offers no interactive transaction was wrong, and the optimistic-concurrency design built on it has been replaced with genuine `SELECT ... FOR UPDATE` locking.

This plan starts from all of the above as given. What's left is real but narrower than it was: three gaps found reviewing the review (§6.1, headlined by `Tx:AddQuantity` skipping every check except slot capacity), what the robbery system needs from `feather-core` to turn on at all, and the UX/feature backlog in §10.
...
server/services/{instances,transactions,guards,equipment}.lua                                 (InstancesAPI / TransactionAPI / GuardsAPI / EquipmentAPI — the INV-W contract layer)
server/helpers/result.lua                                                                     (Result envelope + stable code catalog)
       │
       ▼
server/{controllers,services}/slots.lua + controllers/{inventory,item,ground,categories}.lua   (raw MySQL — inventory/inventory_items/items/categories/inventory_blacklist/ground/inventory_access/character_equipment)
```
...
- `ItemsAPI` — `GetDefinitions`, `GrantItem` (trusted, atomic), `AddItem`, `RemoveItemByName`, `RemoveItemById`, `GetItem`, `GetItemCount`, `ItemExists`, `InventoryHasItems`, `RegisterUsableItem`, `UseItem`, `DropItemsOnGround`, `Get/Set/AdjustCondition`
- `CategoriesAPI` — `GetCategories`
- `InstancesAPI` (INV-W1) — `GetInstance`, `FindInstances`, `GetItemForCharacter`, `ReadMetadata`, `WriteMetadata`, `MergeMetadata`, `IsUniqueDefinition`, `SetInstanceMode`, `GetCapabilities`
- `TransactionAPI` (INV-W2), surfaced as `Transaction` / `MutateItem` / `CreateInstance` — plus `Diagnostics.GetTransactionMetrics`
- `GuardsAPI` (INV-W3) — `RegisterMoveGuard`/`RegisterDestroyGuard`/`Unregister*`, `CanMoveInstance`/`CanDestroyInstance`, and the `Feather:Inventory:*` post-commit event names
- `EquipmentAPI` (weapons review, 2026-08-24) — `GetEquippedForCharacter`, `SetEquippedForCharacter`, `ClearEquippedInstance`, `IsInstanceEquipped`
- Flat provider surface for `feather-weapons`' adapter, aliased at the top level under exactly the names it expects: `GetCapabilities`, `GetItemForCharacter`, `GetEquippedForCharacter`, `SetEquippedForCharacter`
...
- Events: `Feather:Inventory:ItemCreated`/`ItemMoved`/`ItemMetadataChanged`/`ItemDestroyed`/`TransactionCommitted`. The legacy `feather-inventory:ItemAdded`/`ItemRemoved` pair was removed 2026-08-24 (§6.2)

Schema additions since the last version of this plan: `inventory.owner_character_id`, `inventory.is_public`, `inventory_items.slot_index`, `inventory.max_slots`, `items.instance_mode`, `inventory_items.metadata`/`metadata_revision`/`row_revision`, the `inventory_access` and `character_equipment` tables, and the widening of `items.weight` → `DECIMAL(6,2)` / `inventory.max_weight` → `DECIMAL(8,2)` — all self-migrated at startup the same way `RegisterForeignKey` always has (`SHOW COLUMNS` + conditional `ALTER TABLE`), never touching `feather-recipe`'s own migration.
...
Two constraints this exposed. The first is now **closed**:

- ~~**`items.weight` is an `int` column**, so 1 lb is the smallest expressible weight.~~ **Fixed 2026-08-24.** `items.weight` is `DECIMAL(6,2)` and `inventory.max_weight` is `DECIMAL(8,2)`, self-migrated in `instances.lua`'s `EnsureInstanceSchema` and guarded on the current type so it runs once. `DECIMAL` and deliberately **not** `FLOAT`: weights are `SUM`med across an inventory and compared against a limit, and binary floating point accumulates error that makes that comparison unreliable exactly at the boundary — the same reasoning the schema already applies to money and coordinates. Widening `INT` → `DECIMAL` is lossless (`1` becomes `1.00`), so no data migration was needed. An apple can now weigh `0.25`.
...
### 6.1 Weapons-review remediation (2026-08-24) — and one premise that was simply wrong

The `feather-weapons` review landed a second pass over the whole INV-W track. Most of it is ordinary hardening, but one item is a correction to this plan rather than a change to the code, and it is recorded here rather than quietly edited into §11:

- **`oxmysql` DOES expose an interactive transaction. The previous claim that it does not was wrong, and it came from the wrong source.** `INV-W2` below was written against `.luals/oxmysql.lua` — a hand-written *editor type stub* that stops at `MySQL.transaction.await` — instead of against oxmysql's actual source. `MySQL.startTransaction(cb)` exists and gives a dedicated connection, a `query` bound to it, and rollback by returning `false`. Everything the plan said was unachievable is achievable. **Real pessimistic locking replaced the optimistic guard-statement design**: `SELECT ... FOR UPDATE` inside the transaction, decide, write, commit. Contending callers now queue on the row lock instead of racing and being told `CONFLICT`. Revision compare-and-set is *kept on top* of locking, because a lock spans one transaction while a revision spans a conversation across requests. §11's `INV-W2` and §12's caveat are corrected accordingly. Carried caveat: oxmysql logs `startTransaction is "experimental"`, so the calling convention is isolated in exactly one function (`RunInTransaction`) and exercised end-to-end by `/InvTxSmokeTest` under `Config.DevMode`.
- **`row_revision` joined `metadata_revision`.** A metadata-only revision was insufficient: a caller could read ammunition, a concurrent request could *move* that ammunition to a different inventory without touching metadata, and the first caller's compare-and-set would still pass — then consume ammo from its new owner. `row_revision` moves for any change to the row, moves included. Kept separate rather than replacing, so a consumer that genuinely only cares whether the *document* changed can still ask that narrower question.
- **`MoveInventoryItems` is now atomic, with capacity evaluated inside the transaction against locked rows.** The pre-check previously ran outside any transaction, so two concurrent transfers could both pass it and together overfill the destination; the per-item `UPDATE`s were independent, so a failure partway left some items moved and some not. `AcceptanceInTransaction` exists because `EvaluateInventoryAcceptance` uses `MySQL.query.await` — a *different* connection — and calling it while holding row locks would have the transaction waiting on itself.
- **Every legacy removal path now asks the destroy guards.** `RemoveItemByName`/`RemoveItemById` previously called the raw delete, so an equipped weapon could be destroyed through a legacy API with no chance to veto — the guard registry existed but only the new paths used it. All-or-nothing on `RemoveItemByName`: a partial removal because one unit was vetoed would be worse than refusing the request.
- **`ItemAdded`/`ItemRemoved` now always carry an instance id.** `GrantItem` fired the event with a *definition* id while the movement paths fired it with an `inventory_items.id` — the same event carrying two different kinds of identifier depending on which route produced it, so a consumer could not tell what it had been handed. `GrantItem` now uses `INSERT ... RETURNING id`.
- **The flat `item_metadata` table is no longer read or written** (see §11 `INV-W1`, whose earlier decision is now reversed).
- **Persisted equipment slots** (`character_equipment`) — generic `character × slot → instance`, where `slot` is an arbitrary string the consumer names (`'primary'`, `'holster'`, `'hat'`). Inventory stores and constrains it; the consumer decides what the names mean, so a clothing resource can use the same table without inventory learning about clothing. The load-bearing point from the review: this cannot live in weapons' memory, because it has to survive a reconnect and a restart. Ownership is re-derived from the instance's own row rather than trusted, and the `inventory_items` FK cascades, so "consumed while equipped" self-heals instead of leaving a dangling reference.
- **`Tx:RequireAccess`** — when a transaction context names an `actorSource`, every mutating operation asserts that actor's *live* access to the inventory it touches, per operation rather than once per transaction (a transaction can touch several inventories, and access to one is not access to another). A context with no `actorSource` is a trusted server-side operation and is not gated.
- **`MutateItem` / `CreateInstance`** exist because a Lua transaction handle carrying methods cannot cross a Cfx export boundary, only data can. They accept a specification and run the locked mutation entirely inside this resource.
...
### 6.2 Backwards-compatibility removal, group A (2026-08-24)

The framework is alpha and the compatibility policy at the top of this document already permits breaking changes. An audit found eight surfaces that existed *only* to avoid breaking consumers; five of them had no consumer at all, and those five are now removed. The other three (the dual result convention, the duplicate `AddItem`/`GrantItem` grant paths, and the numeric-src/string-UUID `inventoryId` guessing) do have call sites in `feather-weapons` and `feather-admin` and are deliberately left for a coordinated pass.

Verified before removal by reading `feather-weapons/server/adapters/feather_inventory.lua` and `feather-admin/server/services/inventory.lua` in full, and searching every Lua file in `feather-core`, `feather-character` and `feather-menu`.

- **Legacy `feather-inventory:ItemAdded`/`ItemRemoved` removed** — all eleven fire sites. Every one already sat beside a structured `Feather:Inventory:*` emit carrying strictly more (correlation id, actor, definition id, revision), and the legacy pair's first argument had genuinely meant two different things depending on route until recently. No `AddEventHandler` for either name existed anywhere in the framework.
- **`ItemsAPI.SetMetadata` removed.** It had become a passthrough to `InstancesAPI.MergeMetadata` that made the result *worse* on the way out — unwrapping the envelope into `{ error, code, message }` and discarding the correlation id.
- **`metadata_revision` collapsed into `row_revision`.** It was retained so a consumer could ask the narrower "did only the document change" question; none ever did, and two counters where one is authoritative made every new compare-and-set a coin flip. **No migration ships for it** — the column is simply no longer created, and this release is delivered with "rebuild your dev database" rather than an in-place upgrade path (decision 2026-08-24). An earlier draft seeded the old value forward and dropped the column; that was solving for a schema-upgrade guarantee this framework does not offer at alpha. On a database that already has the column it is left behind, unread and unwritten, defaulting to 0 on insert. The capability flag renamed `metadataRevision` → `instanceRevision` — the one externally-visible rename in this pass, verified unread by both consumers.
- **`GetItemCount`'s `-1` sentinel replaced with `nil`.** `-1` is a plausible-looking number that survives arithmetic, so a failure picked a different wrong answer depending on how the caller asked: `count > 0` read it as "has none", `count ~= 0` read it as "has some".
- **The `item_metadata` payload alias renamed to `metadata`.** This resource no longer refers to the flat table's name in any form.

**One item was scoped down rather than done, and the reason matters.** The plan was to drop the dead flat `item_metadata` table. It is created by **`feather-recipe`'s migration**, not by this resource — the same migration §5 says this resource must never touch. A `DROP TABLE` from here would be both an ownership violation and futile, since `feather-recipe` would recreate it on its next run.

**Resolved 2026-08-24: `feather-recipe` stops creating the table.** Removing it at the source is the correct fix rather than deleting it downstream, and it is the same team's call to make either way — the constraint was never about who owns the decision, only about which resource's code should carry the statement. This resource's own references to the table are gone, and it issues no `DROP TABLE` for anything.

The rule this leaves in place: **a resource creates the columns it owns and stops creating the ones it no longer needs.** After the decision above, `feather-inventory` issues no `DROP TABLE` and no `DROP COLUMN` at all — schema removal happens by a resource ceasing to create something, plus a database rebuild.

**A latent gap surfaced by the removal, now fixed.** `MoveSlotItemsPartial` — the cross-inventory stack-merge path — emitted **no structured event at all**. That was invisible for as long as the legacy pair fired in its place; deleting them exposed it. The merge path now emits `ItemMoved` with the definition id and the acting source, and it also bumps `row_revision`, which it was not doing — a merge relocates an instance, so a compare-and-set holding a pre-merge revision has to conflict. This is a real correctness fix that the compat layer had been masking.

## 7. Supporting feather-weapons (target contract from `DEPENDENCY_SUPPORT_PLAN.md` §4)

Full detail lives in `DEPENDENCY_SUPPORT_PLAN.md`. **This section is now a status table rather than a work list — every line of the target contract is built** (`INV-W1`–`W4`, §11). What remains is the §6.1 gap list and a vertical-slice test that cannot run until `feather-weapons` exists.

**Item definitions.** ✅ `items.instance_mode` (`stack`/`unique`), backfilled from `max_stack_size <= 1`, enforced at `GetJoinableSlot` — the single placement chokepoint, so "unique cannot stack" covers `AddItem`, `GrantItem`, `MoveInventoryItems` and `Tx:AddQuantity` at once rather than as four separately-maintained checks.

**Item instances.** ✅ `InstancesAPI.GetInstance` / `FindInstances` / `GetItemForCharacter` return one normalized shape (instance id, definition, container, slot, metadata document, `revision` + `metadataRevision`). Identity is preserved across movement — rows are updated in place, never delete-and-recreate.

**Metadata.** ✅ `inventory_items.metadata` (JSON) + `metadata_revision`, replaced-whole in one statement. The flat `item_metadata` table is neither read nor written any more. `ItemsAPI.SetMetadata` routes through `MergeMetadata`, so the legacy export inherits atomicity and versioning without changing its own shape.

**Transactions and locks.** ✅ `Inventory.Transaction(context, fn)` on real `MySQL.startTransaction` with `SELECT ... FOR UPDATE` row locking (see §6.1 — the earlier "not achievable" claim was wrong). Compare-and-set is retained on top for reads that span requests. `MoveInventoryItems` and `MoveSlotItems` no longer check-then-write outside a transaction.

**Idempotency and guards.** ✅ Idempotency keys (bounded in-memory, 500 entries / 60s — **does not survive a restart**, so anything economic needs its own persisted record). ✅ `GuardsAPI` pre-move/destroy registry, wired at the chokepoints and now also on the legacy removal paths. Item-instance *leases* were not built and are not needed: real row locking covers what leases were proposed to approximate.

**Events.** ✅ `ItemCreated`/`ItemMoved`/`ItemMetadataChanged`/`ItemDestroyed`/`TransactionCommitted`, internal `TriggerEvent` (never network-registered — a networked mutation event is spoofable), emitted only post-commit. Legacy `ItemAdded`/`ItemRemoved` still fire alongside, now always carrying an instance id. ⚠️ `ItemMoved` payload is incomplete on the non-transactional path — §6.1.

**Container access.** ✅ `InventoryAPI.CanAccessInventory(src, inventoryId, action, context)` with `READ`/`INSERT`/`REMOVE`/`MANAGE`, layered over `inventory_access.lua`'s real ACL rather than duplicating it — the question §7 left open is answered: weapons calls inventory's own model through this function, and inventory did not build a second permission system. `Tx:RequireAccess` applies it per operation inside a transaction.

**Equipped state.** ✅ `character_equipment` + `EquipmentAPI` — not in the original contract, added by the review because equipment cannot live in a consumer's memory and survive a restart.

## 8. Concurrency and locking

**Superseded 2026-08-24.** The prior text described a resource with no transaction wrapping and no lock on concurrent mutation. That is no longer the shape: `MoveInventoryItems`, `MoveSlotItems` and everything on the `Transaction` API run inside a real database transaction with `SELECT ... FOR UPDATE` on the rows they touch, so concurrent mutation of the same inventory queues on the row lock rather than racing.

What survives from the old text: `OpenInventories` still locks a non-owned inventory to one `src` while it is open. That is a UI-level exclusivity lock, unrelated to the database locking above, and it is still in-memory only — empty on boot, which means *unlocked*, which is the safe direction. Do not build a standalone mutex; there is now nothing left for one to do.
...
- ~~Design and migrate the versioned metadata document; decide whether flat `item_metadata` survives.~~ `inventory_items.metadata` (JSON) + `metadata_revision`. **Decision REVERSED 2026-08-24: the flat table does not survive.** The original decision here was a read-only compatibility projection — reads `COALESCE`ing the document over a `JSON_OBJECTAGG` of the flat rows. The weapons review removed it, and the reasoning holds: *nothing consumed the flat shape*, so carrying a second read path for a projection no reader wanted was pure cost, and a per-key write has neither atomicity nor a revision. The table is left in place unused rather than dropped, so a server owner with their own reader is not broken by a resource update; this resource neither reads nor writes it.
- ~~Add a general instance revision.~~ (Weapons review, 2026-08-24) `inventory_items.row_revision`, bumped by any row change including moves — see §6.1 for why a metadata-only revision was insufficient. **`metadata_revision` was then removed entirely (§6.2)**: it was kept only so a consumer could ask the narrower question, and none did. `row_revision` is now the single instance revision.
...
### `INV-W2` — Transactions and concurrency — **DONE 2026-08-23, REWRITTEN 2026-08-24**

**The premise this phase was originally written against was wrong, and the correction is the headline.** The first implementation claimed `oxmysql` exposed no interactive transaction — only `MySQL.transaction.await(queries)`, an array of statements — and concluded that "deterministic multi-item row locking is not achievable against this dependency." That claim came from `.luals/oxmysql.lua`, a hand-written *editor type stub*, not from oxmysql's source. `MySQL.startTransaction(cb)` exists: a dedicated connection, a `query` bound to it, arbitrary Lua between reads and writes, and rollback by returning `false`. Everything said to be missing was there. Recorded rather than silently corrected, because the same failure mode — validating a dependency's capability against a stub — is what `DEPENDENCY_SUPPORT_PLAN` §12 asks to be checked.

- ~~Implement `Inventory.Transaction(context, fn)`.~~ Genuine pessimistic locking: `SELECT ... FOR UPDATE` inside the transaction, decide in Lua, write, commit. Contending callers **queue on the row lock** instead of racing and retrying, and capacity is re-checked inside the same transaction that commits the move — which is what stops two concurrent transfers from both passing a pre-check.
- **Compare-and-set is kept on top of locking, not replaced by it.** A lock spans a single transaction; a revision spans a conversation. A caller that reads an instance in one request and writes it in a later one still needs `expectedRevision`, compared against `row_revision` so a *move* between the two also conflicts.
- **The rollback-detection bug the review caught.** `pcall` returns `(executed, value)`; reading only the first is how a transaction that returned `false` *without raising* was treated as success — post-commit events emitted for a rollback. Both are captured now, and `executed` is checked before `committed` is trusted.
- ~~Add idempotency records.~~ Bounded in-memory cache (500 entries, 60s TTL). Not a table: the record only needs to outlive a request's retry window. **Stated limitation:** it does not survive a restart, so a retry across one *will* execute again. Acceptable for a reload or a repair; not acceptable for anything economic, which needs a persisted record the caller owns keyed on its own domain.
- **Cross-resource boundary.** A Lua transaction handle carrying methods cannot cross a Cfx export; only data can. `MutateItem(context, spec)` and `CreateInstance(context, spec)` take a specification and run the locked mutation entirely inside this resource.
- **Exit gate — `/InvTxSmokeTest` (DevMode), six cases run in-game rather than asserted:** create + metadata + locked read in one transaction; rollback leaves nothing behind; a stale revision is refused; a row *deleted* mid-flight reports `NOT_FOUND` rather than silently succeeding; a row *moved* mid-flight conflicts via `row_revision`; and a legacy `RemoveItemById` respects a registered destroy guard.

**Carried caveat:** oxmysql logs that `startTransaction` is experimental and may receive breaking changes. The calling convention is therefore isolated in exactly one function (`RunInTransaction`), so an upstream change is a one-function fix rather than a rewrite.
...
- ~~Failure diagnostics and transaction metrics.~~ Counters exposed via `Diagnostics.GetTransactionMetrics`, returned as a copy so a caller cannot reset them by mutating what it was handed. **Updated 2026-08-24** for the locking rewrite: the set is now `started`/`committed`/`rolledBack`/`conflicts`/`bodyErrors`/`idempotentHits`. `retriesExhausted` is gone along with the retry loop it counted — with real locking, contending callers queue rather than retry. The number that matters operationally is now `rolledBack`, and `conflicts` is a genuine signal again rather than expected background noise, since it can only come from a cross-request compare-and-set. Failures log with the correlation id so a line can be tied back to its originating request.
- ~~Restart tests.~~ Audited rather than asserted: the only in-memory state is `OpenInventories`, `TemporaryGrants`, the idempotency cache and the metrics counters. Each is empty on boot, and empty means *deny* (grants), *unlocked* (open-inventory locks), *cache miss* (idempotency) and *zero* (metrics). **No in-memory structure is persistent authority** — the §12 checklist item — and every one fails safe rather than open. Equipment moved *out* of this list by being persisted (`character_equipment`), which is the point of §6.1's equipment item.
- **Exit gate:** the vertical-slice suite belongs to `feather-weapons` and cannot run before that resource exists. What is verifiable here is verified: unique instances cannot stack, compare-and-set yields one commit and one explicit conflict, guards veto before any write, events fire only post-commit, and all 34 Lua files parse clean.

**Concurrency note, corrected 2026-08-24:** this is now pessimistic locking, not optimistic concurrency. `oxmysql` does expose an interactive transaction (`MySQL.startTransaction`), so contending callers queue on the row lock rather than conflict-and-retry. See §6.1.

### Inventory-Native Phase A — Close the loop (§10.1)
- ~~`MoveItem` weight/capacity bypass~~ — **both halves done**; `EvaluateSlotMove`'s net-delta math covers the occupied-slot swap and closed its previously-unvalidated return leg (§6).
- Unblock robbery — **pending by design**, not merely open: the statuses it gates on are client-authoritative in RedM, so this waits on an authoritative model in `feather-core`, not on someone writing `HasStatus` (§10.1).
- ~~`AddItem` weight parity~~, ~~`GiveItem` distance check~~, ~~surfaced rejection reasons~~, ~~debug print cleanup~~, ~~split-stack action~~ — done.
...
### Phase D — Public API documentation — **DONE 2026-08-24**
- ~~Document every export's real contract once `INV-W1`–`W4` stabilize it.~~ `README.md`'s API Reference now carries a worked example for every export on `Inventory`, `Items`, `Categories`, `Instances`, `Transaction`, `Guards`, `Equipment` and `Diagnostics`, plus the top-level provider aliases, both event families, and the client surface. Written against the source rather than against this plan — which is how §6.1's three gaps were found.
- Documented honestly rather than aspirationally: the two open gaps (`Tx:AddQuantity`'s missing checks, `ItemMoved`'s incomplete payload) are called out **inline at the API they affect**, not only in the To-Do list, so a consumer reading the reference cannot build against a contract this resource does not yet honour.
...
- [x] Multi-item flows can commit atomically through `Transaction(context, fn)` (INV-W2) — real `MySQL.startTransaction`, rewritten 2026-08-24
- [x] Concurrent requests cannot duplicate state (INV-W2) — `SELECT ... FOR UPDATE` row locking; contending callers queue rather than conflict. Cross-request compare-and-set against `row_revision` covers reads that span requests
- [x] A move invalidates a stale compare-and-set (INV-W1, weapons review) — `row_revision` moves for any row change, not just document writes
...
- [x] `ItemsAPI.SetMetadata` removed — passthrough that discarded the envelope and correlation id
- [x] `metadata_revision` collapsed into `row_revision` — no longer created; no in-place migration, rebuild the database
- [x] `GetItemCount`'s `-1` sentinel replaced with `nil`
...
- [x] Every export documented against its real contract (Phase D, 2026-08-24) — README API Reference, one worked example per export, written from source
