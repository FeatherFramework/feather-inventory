...
- **Seeded weights are effectively placeholder** — 65 of 66 items weigh `1`, the remaining one weighs `2`. An Elephant Rifle, a Cattleman Revolver and an apple are all 1 lb. While the limit was unreachable this didn't matter; at 125 lb it decides gameplay, and the practical effect right now is "you may carry 125 things, whatever they are." Giving items real weights (`feather-recipe`'s `seed.sql`, plus an `UPDATE` for existing databases) is what actually makes the limit meaningful — the config change alone only makes it *reachable*.
